...
public class AudioSenderClient {
    ...
    public AudioSenderClient(String userName, String jwtToken, String transcriptionLanguage) {
    ...
    public void connect(String uri) throws DeploymentException, IOException {
        ...
        // 이 클래스(@ClientEndpoint 어노테이션이 붙은)의 인스턴스를 핸들러로 사용하여 서버에 연결
    ...
    public void onMessage(String message){
        try{
            ...
            if ("auth_ok".equals(response.get("type"))) {
                ...
                log.info("AudioSenderClient: User " + this.userName + " registered successfully.");
                registerLatch.countDown(); // isReady()에서 대기 중인 스레드를 깨운다.
            }
        }catch (IOException e) {
            System.err.println("AudioSenderClient: Failed to process or understand message. Ignoring. Error: " + e.getMessage());
        }
    }
    ...
    public void onClose(Session session, CloseReason closeReason) {
        log.info("AudioSenderClient: Connection closed: {}", closeReason);
    }
    ...
    public void onError(Session session, Throwable throwable) {
        log.error("AudioSenderClient: An error occurred.", throwable);
    }
    ...
    public void sendAudio(byte[] audioData) throws IOException, InterruptedException {
        if (this.session != null && this.session.isOpen()) {
            ...
            while (offset + CHUNK_SIZE <= floatBytes.length) {
                ...
                Thread.sleep(chunkIntervalMillis); // Simulate streaming
            }

            // Handle the final partial chunk by padding it to CHUNK_SIZE
            int remainingBytes = floatBytes.length - offset;
            if (remainingBytes > 0) {
                log.info("AudioSenderClient: Sending final padded chunk of size " + remainingBytes);
                ...
                finalChunk.rewind(); // Prepare buffer for sending
                this.session.getBasicRemote().sendBinary(finalChunk);
            }

            log.info("AudioSenderClient: Finished sending audio.");

            // --- VAD Status (speaking: false) 메시지 전송 로직 추가 ---
            log.info("AudioSenderClient: Sending VAD status 'speaking: false' message...");
            // 4. 전송할 JSON 메시지 생성
            ...
            log.info("AudioSenderClient: VAD status message sent.");
            Thread.sleep(1000);
        }
    }
